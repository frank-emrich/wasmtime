...
pub(crate) fn emit(
    ...
    match inst {
        ...
        Inst::StackSwitch { .. } => {
            // Location to which someone resuming this stack will jump to:
            // Right behind the StackSwitch instruction
            let resume = sink.get_label();

            let amode = Amode::RipRelative { target: resume };
            let inst = Inst::lea(amode, Writable::from_reg(regs::rax().into()));
            inst.emit(sink, info, state);

            //
            // Begin code generated by generate_stack_switch_assembly.sh
            //
            ...
            sink.put_data(&[0x55]);
            // mov    rbp,rsp:
            sink.put_data(&[0x48, 0x89, 0xe5]);
            // push   rbx:
            sink.put_data(&[0x53]);
            // push   r12:
            sink.put_data(&[0x41, 0x54]);
            // push   r13:
            sink.put_data(&[0x41, 0x55]);
            // push   r14:
            sink.put_data(&[0x41, 0x56]);
            // push   r15:
            sink.put_data(&[0x41, 0x57]);
            // mov    rax,QWORD PTR [rsi]:
            sink.put_data(&[0x48, 0x8b, 0x06]);
            // mov    QWORD PTR [rdi],rbp:
            sink.put_data(&[0x48, 0x89, 0x2f]);
            // lea    rsp,[rax-0x28]:
            sink.put_data(&[0x48, 0x8d, 0x60, 0xd8]);
            // pop    r15:
            sink.put_data(&[0x41, 0x5f]);
            // pop    r14:
            sink.put_data(&[0x41, 0x5e]);
            // pop    r13:
            sink.put_data(&[0x41, 0x5d]);
            // pop    r12:
            sink.put_data(&[0x41, 0x5c]);
            // pop    rbx:
            sink.put_data(&[0x5b]);
            // pop    rbp:
            sink.put_data(&[0x5d]);
            // add    rdi,0x10:
            sink.put_data(&[0x48, 0x83, 0xc7, 0x10]);
            // mov    rax,r8:
            sink.put_data(&[0x4c, 0x89, 0xc0]);
            // ret:
            sink.put_data(&[0xc3]);

            //
            // End code generated by generate_stack_switch_assembly.sh
            //
