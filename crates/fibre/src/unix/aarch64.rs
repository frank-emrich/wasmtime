...
use super::wasmtime_fibre_start;
...
asm_func!(
    wasmtime_versioned_export_macros::versioned_stringify_ident!(wasmtime_fibre_switch),
    concat!(
        "
            .cfi_startproc
        ",
...
// from wasmtime_fibre_start(). As a result, we need a signed address
...
//   wasmtime_fibre_switch(), otherwise the latter would fault
...
// wasmtime_fibre_start(), and provides wider coverage.
#[rustfmt::skip]
asm_func!(
    wasmtime_versioned_export_macros::versioned_stringify_ident!(wasmtime_fibre_init),
    concat!(
        "
            .cfi_startproc
            hint #34 // bti c
            sub x16, x0, #16
            adrp x17, ", sym_adrp!("{fibre}"), "
            add x17, x17, ", sym_add!("{fibre}"), "
        ",
        paci1716!(),
        "
            ...
            // `wasmtime_fibre_switch` has an 0xa0 byte stack, and we add 0x10 more for
    ...
    fibre = sym wasmtime_fibre_start,
);
...
// match the frame layout in `wasmtime_fibre_switch`.
asm_func!(
    wasmtime_versioned_export_macros::versioned_stringify_ident!(wasmtime_fibre_start),
    "
