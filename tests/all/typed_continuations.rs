    ...
    pub fn make_i32_inc_via_export_host_func(
        ...
        Func::new(
            ...
            |mut caller, args: &[Val], results: &mut [Val]| {
                let export = caller
                    .get_export(export_func)
                    .ok_or(anyhow::anyhow!("could not get b"))?;
                let func = export
                    .into_func()
                    .ok_or(anyhow::anyhow!("b is not a Func"))?;
                let func_typed = func.typed::<i32, i32>(caller.as_context())?;
                let arg = args[0].unwrap_i32();
                let res = func_typed.call(caller.as_context_mut(), arg + 1)?;
                results[0] = Val::I32(res + 1);
...
mod host {
    ...
    #[test]
    /// Similar to `call_host_from_continuation`. However, the host function in turn
    /// calls into wasm and runs a continuation.
    /// Call chain:
    /// $entry -resume-> $a -call-> $host_func_a -call-> $b -resume-> $c
    fn call_host_from_continuation_nested_ok() -> Result<()> {
        let wat = r#"
        (module
            (type $ft (func (param i32) (result i32)))
            (type $ct (cont $ft))
    ...
    #[test]
    /// Similar to `call_host_from_continuation_nested_ok`. However, after
    /// crossing from the host back into wasm, we suspend to a tag that is
    /// handled by the surrounding function (i..e, without needing to cross the
    /// host frame to reach the handler).
    /// Call chain:
    /// $entry -resume-> $a -call-> $host_func_a -call-> $b -resume-> $c
    ...
    #[ignore] // Needs clarification about intended behavior. Do host functions act as barriers for suspensions?
    #[test]
    /// Similar to `call_host_from_continuation_nested_suspend_ok`. However,
    /// we suspend to a tag that is only handled if we were to cross a host function
    /// boundary.
    ...
    fn call_host_from_continuation_nested_suspend_unhandled() -> Result<()> {
        ...
        (module
            ...
            (func $entry (export "entry") (result i32)
                (block $h (result (ref $ct))
                  (return (resume $ct (tag $t $h) (i32.const 123) (cont.new $ct (ref.func $a))))
                )
                (drop)
                (i32.const 123)
            )
...
mod traps {
    ...
    #[test]
    /// Tests that we get correct backtraces if we trap after returning from one continuation to its parent.
    /// Call chain:
    /// $entry -call-> $a -resume-> $b -call-> $c -resume-> $d -call-> $e
    /// Here, $f traps.
    ...
    #[test]
    /// Tests that we get correct backtraces if we trap after returning from several continuation back to the main stack.
    /// Call chain:
    /// $entry -call-> $a -resume-> $b -call-> $c -resume-> $d -call-> $e
    /// Here, $f traps.
    ...
    #[test]
    /// Tests that we get correct backtraces after suspending a continuation and then resuming it from a different stack frame.
    /// Call chain:
    /// $entry -call-> $a -resume-> $b -call-> $c -resume-> $d -call-> $e
    /// Here, $f traps.
    ...
    #[test]
    /// Tests that we get correct backtraces after suspending a continuation where we need to forward to an outer handler
    /// Call chain:
    /// $entry -call-> $a -resume-> $b -call-> $c -resume-> $d -call-> $e
    /// Here, $f traps.
    fn trap_in_continuation_forward() -> Result<()> {
        ...
        (module
            ...
            ;; In this test, the "unreachable" trap is the one we deliberately
            ;; trigger to produce the backtraces we want. Thus, to indicate that
            ;; the test went somewhere we genuinely want to consider
            ;; unreachable, we trigger this alternative trap, by calling a null
            ;; ref.
            (func $alternative_trap
                (call_ref $ft (ref.null $ft))
            )
    ...
    #[test]
    /// Tests that we get correct backtraces after suspending a continuation where we need to forward to an outer handler.
    /// We then resume the continuation from within another continuation.
    /// Call chain:
    /// $entry -call-> $a -resume-> $b -resume-> $c -resume-> $d -call-> $e
    /// Here, $f traps.
    fn trap_in_continuation_forward_resume() -> Result<()> {
        ...
        (module
            ...
            (func $b (export "b")
                ...
                (global.set $k)
                (resume $ct (cont.new $ct (ref.func $f)))
                ;; We obtain the continuation (where forwarding happended), and resume it from another one
            )
    ...
    #[test]
    /// Tests that we get correct backtraces if we panic deep inside multiple continuations.
    /// Call chain:
    /// $entry -call-> $a -resume-> $b -call-> $c -resume-> $d -call-> $e -call-> $f
    /// Here, $f is a host function that panics.
    ...
    "#;
        ...
        // TODO: Check actual backtrace frames
